...
async def list_effective_work_types(
    ...
    try:
        ...
        cache_tag = await get_cache_tag(redis, tenant_effective_cache_scope(org_id))
...
async def get_catalog_work_type_detail(
    ...
    await set_cached(redis, cache_key, payload, GLOBAL_CATALOG_TTL_SECONDS, tag=cache_tag)
    return payload
...
async def get_effective_work_type(
    ...
    try:
        ...
        payload = item.model_dump(mode="json")
        await set_cached(redis, cache_key, payload, TENANT_EFFECTIVE_TTL_SECONDS, tag=cache_tag)
        return payload
    finally:
        await enforce_timing_floor(
            started_at,
...
async def get_project_work_item_effective_configuration(
    ...
    try:
        ...
        payload = item.model_dump(mode="json")
        await set_cached(redis, cache_key, payload, WORKFLOW_CONFIGURATION_TTL_SECONDS, tag=cache_tag)
        return payload
    finally:
        await enforce_timing_floor(
            started_at,
