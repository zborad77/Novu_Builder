...
from app.api.deps import (
    get_quote_recalculation_command_service,
    get_current_user,
    get_project_service,
...
async def recalculate_case_estimates(
    ...
    commands: QuoteRecalculationCommandService = Depends(
        get_quote_recalculation_command_service
    ),
) -> QuoteVariantRecalculateResponse:
    org_id = resolve_org_id(current_user)
    try:
        create_result = await commands.handle(
            RequestQuoteRecalculationCommand(
                case_id=case_id,
    ...
    except QuoteRecalculationCommandError as exc:
