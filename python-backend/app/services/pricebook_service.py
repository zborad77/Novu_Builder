...
def to_pricebook_read(pricebook: PricingProfile) -> PricebookRead:
    return PricebookRead(
        ...
        hourlyRate=float(pricebook.hourly_rate),
        dailyRate=float(pricebook.daily_rate),
        laborHoursPerSqm=float(pricebook.labor_hours_per_sqm),
        vatPct=float(pricebook.vat_pct),
...
def to_pricebook_item_read(item: MaterialCatalog) -> PricebookItemRead:
    return PricebookItemRead(
        ...
        unit=item.unit,
        normPerSqm=float(item.norm_per_sqm),
        defaultUnitPrice=float(item.default_unit_price),
